...
export default function SalidaView() {
  ...
  const { data, isFetching } = useQuery({
    queryKey: ["logistica_salidas", { anno, mes, almacen, estado }],
    queryFn: () =>
      api.get("logistica/dashboard/", {
        params: { operacion: "S", anno: anno || "%", mes: mes || "%", almacen: almacen || "%", estado: estado || "%", general: "" },
      }).then(r => r.data),
    staleTime: 30 * 1000,
  });
  ...
  const exportToExcel = () => {
    ...
    const headerStyle = {
      fill: { fgColor: { rgb: "E11D48" } }, // Rose-600 (Color del sistema para Salidas)
    ...
    const cellStyleOdd = {
      fill: { fgColor: { rgb: "FFF1F2" } }, // Zebra style (Rose-50)
      font: { name: "Arial", sz: 9, color: { rgb: "1E293B" } },
      border: {
        top: { style: "thin", color: { rgb: "FFE4E6" } },
        bottom: { style: "thin", color: { rgb: "FFE4E6" } },
        left: { style: "thin", color: { rgb: "FFE4E6" } },
        right: { style: "thin", color: { rgb: "FFE4E6" } }
      }
    };
    ...
    XLSX.writeFile(workbook, `salidas_reporte_${new Date().toISOString().slice(0, 10)}.xlsx`);
    toast.success("Reporte de Excel de Salidas generado correctamente");
  };
  ...
  const exportToPDF = () => {
    ...
    doc.text("REPORTE DE SALIDAS DE ALMACÉN", 14, 18);
    
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9);
    doc.setTextColor(255, 228, 230); // Rose-100
    const dateStr = new Date().toLocaleDateString() + " " + new Date().toLocaleTimeString();
    doc.text(`Generado: ${dateStr} | Total registros: ${dataToExport.length}`, 14, 28);

    const columns = ["N° Reg", "Fecha", "O/C", "Destinatario", "Guía", "Almacén", "Soles", "Dólares", "Estado"];
    ...
    toast.success("Reporte de PDF de Salidas generado correctamente");
  };
        ...
        <div className="space-y-1">
          ...
          <div className="relative flex items-center">
            <input 
              ...
              placeholder="N°, destinatario, O/C..." 
              className="w-full text-xs border border-slate-200 rounded-lg pl-3 pr-8 py-1.5 bg-white focus:outline-none focus:ring-1 focus:ring-rose-500 focus:border-rose-500" 
            />
            {searchInput && (
              <button 
